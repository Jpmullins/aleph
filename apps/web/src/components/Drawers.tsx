...
function ModelProfileSection({ projectId }: { projectId: string }) {
  ...
  return (
    <Section title="Model profile">
      ...
      {switchProfile.isPending && <p className="mt-1.5 text-xs text-slate-400">Switching…</p>}
    </Section>
  );
}
    ...
    <li className="rounded-md border border-slate-200 px-3 py-2">
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <div className="truncate text-sm font-medium text-slate-900">{connector.name}</div>

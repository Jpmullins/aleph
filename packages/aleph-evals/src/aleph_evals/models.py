...
class EvalDataset(CommonColumns, Base):
    ...
    gate_thresholds_jsonb: Mapped[dict] = mapped_column(
...
class EvalCase(Base):
    ...
    tags_jsonb: Mapped[list] = mapped_column(
        JSONB, nullable=False, server_default="[]"
    )
    origin: Mapped[str] = mapped_column(
        String(32), nullable=False, server_default="fixture"
    )
...
class EvalRun(CommonColumns, Base):
    ...
    metrics_jsonb: Mapped[dict] = mapped_column(
        JSONB, nullable=False, server_default="{}"
    )
...
class EvalResult(Base):
    ...
    diff_jsonb: Mapped[dict] = mapped_column(
        JSONB, nullable=False, server_default="{}"
    )

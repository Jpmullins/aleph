...
2. **LiteLLM is the only LLM transport.** Every package that needs a model imports `LiteLLMClient` from `aleph-models`. **No provider SDK** (`anthropic`, `openai`, `google-genai`, …) is imported anywhere in Aleph or AIQ. Config: `LITELLM_BASE_URL` + `INSIGHTS_LITELLM_API_KEY` (deployment-level secret).

...
| P4a | Browser chat renders an assistant reply | `tests/playwright/specs/chat-streams-response.spec.ts`, run as `acceptance.sh` P4a — **RED, a known defect under test.** The API path is fine: row H2 drives 10 chat turns over HTTP with a 1.41s first token. The BROWSER path is not — the spec sends a message and no `copilot-assistant-message` ever appears (120s, reproducible), with the captured page showing the assistant dock rendered and neither the user's message nor a reply inside it. Expected-red rather than excluded because `aleph-copilot-runtime` reports HEALTHY throughout: its healthcheck probes `/health`, the one route that is not the product | 🔴 |

...
| **Gateway-driven models + pricing** | **done, proven live** | The live Insights gateway exposed the deepest instance of the organizing principle yet. `pricing.py` shipped a hand-written table of 11 model names asserting it was "verified against the Insights LiteLLM Gateway"; measured against that gateway **every entry was wrong in both dimensions** — the names were unprefixed (`claude-sonnet-4-6` vs `bedrock-claude-sonnet-4-6`, so every call 400s) and the rates were ~3x high (Opus input at $15/MTok against an actual $5.50). And because `cost_for` returned `$0` for an unknown model, the *total* failure was invisible: a full ledger reading $0.00 looks like a quiet day. `test_unknown_model_returns_zero` pinned that behaviour **as the requirement**. Replaced with `aleph_models/discovery.py`: models, capability flags, context windows and exact per-token rates (including absolute cache read/write rates) are read from the gateway's `/model/info` at startup and folded into the live `PricingTable` the client already holds. The committed table is **deleted, not corrected** — a hand-maintained price list is wrong by default. Unpriced is now a recorded state (`pricing_source` ∈ `gateway`/`static`/`unknown`), and `model_calls` gained `cache_write_tokens` + `input_rate_usd`/`output_rate_usd` so a cost is re-derivable from its own row after prices move. **Defaults** come from requirements over discovered metadata (mode, context window, function calling, vision, priced-at-all) then capability/cost ranking — no model name appears in the policy. `autoconfigure` **probes** before binding, which immediately paid for itself: the gateway advertises two Sonnets that both fail on invocation ("Access denied", "on-demand throughput isn't supported"), and the cheapest chat model has an 8k context and no tool calling, so unprobed/unfiltered selection would have bound a broken model to `page_selection` and `extraction`. 70 unit + 9 integration tests over a **verbatim captured `/model/info` payload**. **Live proof:** real call through the real gateway → `cost_usd=0.000067`, `pricing_source=gateway`, rates persisted, cost re-derived from the stored columns and matching. `verify-gateway.sh` rewritten the same way (it asserted 5 model ids, none of which exist). |
| E5 (rest) | open | rail · pipeline strip · card shell · the `!important` shim removal — the multi-week shell rebuild. Note the shim removal is *not* a find-and-replace: `tailwind.config.ts` extends only `fontFamily`, so there is no `bg-surface` to migrate **to** until a Tailwind theme is defined. |

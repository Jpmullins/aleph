...
| **E4.3** generated catalog | **done** | The catalog was maintained by hand in three places with nothing tying them together, and they had drifted in ways no test noticed until an agent hit them. The *correctness* half was closed earlier (three live drifts fixed, guarded by 11 agreement tests). This closes the *structural* half: `packages/aleph-a2ui/src/aleph_a2ui/catalog.json` is now the only editable copy — built by extracting from both live sources so no content was invented or lost — and `catalog.py` (614 → 98 lines) loads it, while `scripts/gen_catalog.py` renders `apps/web/src/a2ui/catalog.ts` and a new `apps/copilot-runtime/src/catalog.generated.ts`. The 265-line inline literal is gone from `server.ts` (333 → 68 lines). `scripts/check-catalog-generated.sh` is CI-wired and **verified to fail in both directions**: hand-editing a generated file, and editing `catalog.json` without regenerating. Two regressions the migration itself introduced were caught by the gates and fixed — the generator initially dropped the `A2UIComponent` type export (web typecheck), and the roster sweep still parsed names out of the deleted literals. Catching drift is weaker than being unable to drift. |
| **E1.3** builder | **done, verified twice** | Drives the real compiled builder graph (no LLM on that path) and reads `report.md` out of the produced ZIP. Reverted once per dropped key: removing `csl_items` yields `(no citations)` in an artifact whose body cites a source — every export shipping an empty reference list while the prose cites; removing `version_no` makes both versions point at `.../1.zip`, so a rebuild overwrote v1's bytes while the row recorded v2 and version history became fiction. **E1 is now closed behaviourally, not just statically.** |
| **E3.4** commit-before-enqueue | **done** | Audited rather than assumed: an AST walk found **six** routes enqueueing inside the open transaction, and one false positive (`reviews.py` writes only a ledger event; the worker creates its own `ReviewRun`), which is now an explicit, reasoned exemption rather than a silent pass. `sources.py:_kick_off_normalize` was the sharpest — its own docstring said "the row is committed by the caller", which was true and happened *after* the job was published. Fixed all five real sites. Also fixed the second clause of the criterion: a missing row raised a plain exception, which **arq treats as terminal**, so a source that lost the race sat in `ingested` forever with nothing recorded to say why; `_missing_row` now defers with backoff for three attempts and then fails loudly, naming the attempt count so an operator can tell a lost race from a deleted row. 15 tests — a static ordering guard (**verified to fail when the commit is removed from `viz.py`**), an anti-vacuity check that the AST walk finds ≥8 dispatchers, and behavioural coverage of the retry policy. |

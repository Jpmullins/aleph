...
- **compose** (LLM, purpose `research.compose`): writes `body_md` citing sources as `[cN]` markers; `aleph_scholar.style_pass` (LLM-free) normalizes the draft; builds a `ResearchReport` dataclass (`topic, body_md, summary, sources, citations_by_marker, claims`).

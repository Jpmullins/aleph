...
5. **Every LLM/tool/embed call writes a `ModelCall` + `CostLedgerEvent`** and is wrapped in an OTEL/Langfuse span. Cost is computed in `aleph_models.pricing` from rates **discovered from the gateway**, and every row records `pricing_source` (`gateway`/`static`/`unknown`) plus the rates used. A model the table cannot price records `unknown`, never a silent `$0`.
...
7. **`ModelProfile` resolves capability → model; the gateway decides what those models are.** Call sites pass a `Capability` and the project's profile; `LiteLLMClient` resolves the binding.

   **Aleph ships no model list and no price list.** `aleph_models.discovery` reads both from the gateway's `/model/info` at startup and folds the rates into the live `PricingTable` the client already holds. This replaced a committed table whose every entry was wrong against the first real gateway: the names were unprefixed (`claude-sonnet-4-6` vs `bedrock-claude-sonnet-4-6`, so every call 400'd) and the rates were ~3x high (Opus input at $15/MTok against an actual $5.50) — and because an unknown model priced at `$0`, the whole failure was invisible behind a plausible $0.00 dashboard.

   Defaults come from `select_default_bindings`, which filters discovered models by *requirements* (mode, context window, function calling, vision, priced-at-all) and only then ranks by capability signals and cost. No model name appears in the policy. `POST /v1/projects/{id}/model-profile/autoconfigure` additionally **probes** each model, because an advertised model is not necessarily reachable — the reference gateway lists two Sonnets that fail on invocation. Capabilities no model satisfies are left unbound and reported.

   The named profiles `aleph-dev` / `aleph-production` remain as presets, selected by `ALEPH_DEFAULT_MODEL_PROFILE`.

...
def build_architecture() -> list[dict]:
    ...
    e.append(
        subtitle(
            40, 70, "The wiki is the primary KB; RKS chunks reached only via intra-source descent"
        )
    )
    ...
    e.append(
        text(x=540, y=445, text="service token + correlation id", font_size=11, color="#495057")
    )
    ...
    e.append(
        text(
            x=320,
            y=455,
            text="connector tools re-enter aleph-api\nfor creds + scope + writes",
            font_size=11,
            color="#495057",
        )
    )
    ...
    steps = [
        ("1. WikiIndex page-selector LLM", "query → top-K wiki page IDs"),
        ("2. Load selected pages", "+ 1-hop wikilink expansion"),
        ("3. Answer composer LLM", "writes answer w/ [[wikilinks]] + [c12]"),
        (
    ...
    e.append(
        text(
            ...
            y=910,
            text=(
                "Solid = direct call.  Dashed = callback / persistence path.\n"
...
def build_domain() -> list[dict]:
    ...
    e.append(
        subtitle(
            40,
            70,
            "RKS upstream → Wiki primary KB → analyst-authored + governance + outputs around it",
        )
    )
    ...
    e.append(
        text(
            ...
            y=720,
            text=(
                "The wiki is the spine. RKS is upstream; analyst-authored and outputs are around it.\n"
...
def build_ui() -> list[dict]:
    ...
    e.append(
        subtitle(
            40,
            70,
            "Left: projects + sessions + bottom icons.  Right: 5 A2UI surfaces (Wiki | Artifacts | Notes | Hypotheses | Briefs)",
        )
    )
    ...
    e.append(
        text(
            x=140,
            y=134,
            text="Project: OSINT — Regional Power Grids 2026",
            font_size=14,
            color="#495057",
        )
    )
    e.append(
        text(x=900, y=134, text="Cost: $4.18 / $50.00    budget OK", font_size=13, color="#2f9e44")
    )
    ...
    for label, active in sessions:
        if active:
            e.append(
                rect(x=48, y=sy - 4, w=204, h=22, bg="#d0ebff", stroke="#74c0fc", stroke_width=1)
            )
    ...
    e.append(
        text(
            ...
            y=698,
            text=(
                "▶ Wiki agent — compiling [[Region X / Infrastructure]] (running)\n"
    ...
    e.append(
        text(
            x=880,
            y=270,
            text="Revision 14 · approved 2026-05-27 · hand-edit on intro §",
            font_size=11,
            color="#868e96",
        )
    )
    ...
    e.append(
        text(
            ...
            y=444,
            text=(
                "Transformer capacity in the regional grid\n"
    ...
    e.append(
        text(x=892, y=716, text="ChartCard (inline on wiki page)", font_size=11, color="#7c4900")
    )
    e.append(
        text(
            x=892,
            y=736,
            text='DatasetVersion: "capacity-3-sources@v4"',
            font_size=11,
            color="#495057",
        )
    )
    ...
    e.append(
        text(
            ...
            y=900,
            text=(
                "• Left = projects + sessions + bottom icons (gear/logs/notifications/profile).\n"

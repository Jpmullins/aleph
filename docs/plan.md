...
| 3 | a self-check probe for every subject | **DONE 2026-08-22, and RUN** — `./scripts/acceptance.sh --self-check` reports **`self-check passed — all 48 mutations were caught`**, exit 0, zero `CANNOT`/`NO-OP`/`UNTESTED`, and `git status --porcelain` empty afterwards so every subject was restored. **0 of 26 sweeps unprobed**, from 7, plus the first three of the ten `scripts/_acceptance/` PROBES, which had the same gap one directory over (the other seven drive a gateway, a browser or the live database). Running the harness is the point: three of the probes added today were wrong in ways only it or a reviewer would show — a `^` anchor that matched nothing under `perl -0`, a `${...}` on a replacement side that perl read as interpolation, and a guard on an unexported variable that would have aborted the whole script under `set -u`. Writing them found two defects in their own subjects: a class declared above the first `{` in a stylesheet was invisible to the dead-CSS sweep (`([^{}]*)\{` glues the `@import` lines onto the selector text and an `if "@" in selector: continue` guard then skips a real rule), and the restore drill — which had no caller at all — found this instance's backup UNRESTORABLE, because postgres inherited Docker's 64 MB `/dev/shm` and pg_restore died building the HNSW index with 40 GB free on the host |
...
- Unanswerable questions produce an honest miss
  <br>`The eval reports an 'abstain rate' on the unanswerable category and it exceeds 0.80. **Measured 2026-08-22 and the mechanism is NOT the problem — the binding is.** Against the live corpus with 8 off-corpus questions, through the real `reranker_for` → `AdaptiveReranker` → `search_corpus` path: `gemma-4-e2b` 0.62, `gpt-oss-120b` 0.62, **`gemma-4-31b` 1.00, `claude-haiku-4-5` 1.00**. Asked directly, the models return exactly the abstention the prompt asks for (`{"relevant": []}`) and `apply_ranking` empties the result. What `gemma-4-e2b` does on its three failures is find a passage relevant — `{"id": 18, "score": 2}` for *how a diesel injector pump is timed*.
  <br>The lever is `CAPABILITY_POLICIES[Capability.RERANK]`, which is `tier="light"`, so autoconfigure binds the cheapest chat model with ≥32k context — a 2-billion-parameter model — to a JUDGEMENT task. Deliberately not flipped here: reranking sits on the latency path of every assistant search, so the tier is a real price and latency trade to measure rather than a bug to fix. Raising `MIN_RELEVANT_SCORE` 1.0 → 2.0 was tried and rejected — measured against the actual replies it recovers one case (0.62 → 0.75, still under the floor) and costs recall on genuine weak hits.`
...
- Claims have stable identity
  <br>Measured FORWARD, the way number 5 is, because an instance-wide count here is dominated by pre-rebuild history and can never reach 0 without editing rows: `select count(*) from wiki_claims where claim_key is null and created_at > <the image that first wrote keys>` returns 0, and re-ingesting the same source does not double the claim count. Print the all-time count beside it rather than scoping it away — `docs/decisions.md` D9. Measured 2026-08-22 after the rebuild: **0 of 83** claims written by the new image have a NULL key; instance-wide 16,100 of 36,194 do, all of them written before it.
...
- Claims are embedded at write time
  <br>``select count(*) from wiki_claims where embedding is null and created_at > <the RS10 image>` returns 0, with the all-time count printed beside it (D9). Measured 2026-08-22 after the rebuild: **83 of 83** claims written by the new image are embedded at write time; instance-wide 263 of 36,194 are, which is what an unscoped version of this criterion would report and why it is scoped. The 786 in the original text was the count when it was written.
...
- No agent tool is left relying on its own try/except for survival.
  <br>``./scripts/check-agent-middleware.sh` exits 0 — an AST walk of the `create_deep_agent(middleware=[...])` call and of every `build_*_subagent`'s returned dict — AND `apps/api/tests/unit/test_agent_tool_guard.py::test_every_subagent_spec_really_carries_the_guard`, which builds every subagent `pkgutil` finds and asserts an `AlephAgentMiddleware` INSTANCE is in the spec. **CORRECTED 2026-08-23.** The criterion used to name `::test_every_registered_tool_is_wrapped` and describe it as building the real graph and enumerating `tool_node.tools_by_name`. It does neither: it drives the middleware over a synthetic handler, passing only each tool's NAME, so the loop over N tools is N repetitions of `test_any_exception_class_becomes_a_tool_message`. Measured — remove `AlephAgentMiddleware` from the orchestrator's middleware list entirely and that test still passes while the sweep goes red. The property is real and held; it was the citation that was wrong.
...
- Events carry subagent identity, so the Inspector can attribute work. This is the H7 requirement, met without the experimental stream.
  <br>`Same file, `::test_subagent_attribution`, driving the request shape deepagents REALLY sends — the subagent's name in `metadata["lc_agent_name"]` and `ls_agent_type: "subagent"` in `configurable` — and asserting `subagent_from_config` returns the name. **CORRECTED 2026-08-23.** The old fixture set `configurable["subagent"]` itself, a key nothing in production writes, so the test supplied the value it then asserted and passed while the field was wrong on every real run: 302 assistant runs, 175 tool events, `orchestrator` the only value ever recorded — including five calls to `deep_read`, a tool defined ONLY inside the retriever subagent. Measured by driving a real delegation against a scripted gateway: `create_sub_agent` passes `name=` to `create_agent`, langchain stamps it as `metadata["lc_agent_name"]`, and `run_name` and `tags` — the two channels the reader used — are empty. The SQL count is not the criterion any more: a distinct-count over a field the fixture fills is a count of the fixture.`

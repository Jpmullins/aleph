...
- Every subagent carries the guard, enforced by a sweep rather than by memory.
  <br>``./scripts/check-agent-middleware.sh` exits 0. It asserts `AlephAgentMiddleware` appears in the `middleware=` list at the `create_deep_agent` call site AND that `grep -c '"middleware"' apps/api/src/aleph_api/subagents/*.py` equals the number of `build_*_subagent` functions (6).`

...
Aleph describes the same 20 components in four envelopes. Three of them are now
generated from one canonical file, and the fourth is not:

1. ``packages/aleph-a2ui/src/aleph_a2ui/catalog.json`` — canonical. The JSON
   Schema the validator enforces for agent-emitted / pinned cards, loaded by
   ``catalog.py``.
2. ``apps/web/src/a2ui/catalog.ts`` — generated: the component + action name
   lists.
3. ``apps/copilot-runtime/src/catalog.generated.ts`` — generated: the schema the
   AGENT is shown, which determines what it believes it may emit.
4. ``apps/web/src/a2ui/aleph-catalog-v09.tsx`` — **hand-written**, the zod3
   render contract, plus the renderers themselves.

``scripts/check-catalog-generated.sh`` proves (2) and (3) still match (1). It
cannot say anything about (4), about the action handlers, or about whether the
vocabulary covers what production writes — which is what this module is for.
Historically these four were all hand-maintained and drifted; two live
consequences of that, neither visible to a name-only comparison:
...
def test_every_declared_action_has_a_handler() -> None:
    ...
    assert not orphaned, (
        ...
        f"{sorted(orphaned)}. The generated-catalog sweep never compares "
        f"declarations against handlers, so nothing else catches this."
    )
...
def test_component_names_agree_between_catalogs() -> None:
    """What `check-catalog-generated.sh` also covers, kept here so this module
    is the single place to look when the catalogs drift."""
    from aleph_a2ui.catalog import _COMPONENTS

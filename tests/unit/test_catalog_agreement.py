...
SERVER_TS = REPO_ROOT / "apps" / "copilot-runtime" / "src" / "server.ts"
...
def _canonical_confidence() -> tuple[str, ...]:
    from aleph_a2ui.catalog import _CLAIM_CONFIDENCE
...
def test_python_catalog_uses_the_canonical_confidence_list() -> None:
    ...
    schema = _COMPONENTS["ClaimCard"]["properties"]["props"]["properties"]["confidence"]
    assert tuple(schema["enum"]) == _canonical_confidence()
...
def test_agent_facing_catalog_matches_the_python_catalog() -> None:
    ...
    """
    m = re.search(
        r"confidence:\s*\{\s*type:\s*\"string\",\s*enum:\s*\[(.*?)\]", SERVER_TS.read_text(), re.S
    )
...
def test_component_names_agree_between_catalogs() -> None:
    ...
    assert ts_names == py_names, (
        ...
        f"only in catalog.py: {sorted(py_names - ts_names)}"
    )
...
def test_agent_catalog_is_valid_json_shaped() -> None:
    ...
    m = re.search(r"const ALEPH_A2UI_CATALOG[^=]*=\s*\{", src)
    assert m, "ALEPH_A2UI_CATALOG not found in the runtime bridge"
